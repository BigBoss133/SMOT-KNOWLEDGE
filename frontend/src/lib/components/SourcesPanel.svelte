...
<div class="h-full flex flex-col p-4 space-y-3">
  <div class="flex items-center justify-between">
    ...
    <span class="text-[10px] text-brain-400 font-mono">
      {sourceList.filter(s => s.active).length}/9 attive
    </span>
  </div>
    ...
    {#each sourceList as source (source.id)}
      <label
        class="flex items-center gap-3 px-3 py-2.5 rounded-lg cursor-pointer
          ...
          {source.active
            ? 'bg-brain-800/50 border-brain-600/40 hover:border-accent-cyan/30'
            : 'bg-brain-800/20 border-brain-700/20 opacity-50 hover:opacity-80'}"
      >
        <input
          type="checkbox"
          checked={source.active}
          onchange={() => toggle(source.id)}
          class="w-4 h-4 rounded border-brain-500 bg-brain-800
            text-accent-cyan focus:ring-accent-cyan/30 focus:ring-offset-0
            accent-accent-cyan"
        />
        <div class="flex-1 min-w-0">
          <div class="flex items-center gap-2">
            <span class="text-sm font-medium text-brain-100">{source.name}</span>
            <span class="text-[10px] px-1.5 py-0.5 rounded-full border {typeColors[source.type]}">
              {typeLabels[source.type]}
            </span>
          </div>
        </div>
        <span class="text-[11px] font-mono {source.active ? 'text-accent-cyan' : 'text-brain-500'}">
          {source.active ? '● attivo' : '○ spento'}
        </span>
      </label>
    {/each}
  </div>
